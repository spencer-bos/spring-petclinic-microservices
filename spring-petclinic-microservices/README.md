...
1) Inside of Eclipse proceed to Help>JRebel>Configuration>Projects
2) Inside of IntelliJ  proceed to Views>Tool Window>JRebel
3) Check all boxes for all services imported into eclipse
4) Proceed to remote servers(same for Eclipse and IntelliJ).
5) Add server for each endpoint url in the server URL section(i.e Server name: API Gateway; Server URL: http://localhost:8080)
6) Add a valid xrebel.lic to the root folder of the spring-petclinic-microservices(same location of the docker-compose.yml)
